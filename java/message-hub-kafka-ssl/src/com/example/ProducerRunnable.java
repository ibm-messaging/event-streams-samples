...
public class ProducerRunnable implements Runnable
{
	 ...
	 * @param topic  	{String}	The topic to post the provided message to.
	 * @param message 	{String}	A stringified version of a JSON object, representing an array of messages to be sent.
	 */
	ProducerRunnable(String broker, String apiKey, String topic)
	{
		this.topic = topic;
		// Create a Kafka producer, providing client configuration.
		this.kafkaProducer = new KafkaProducer<byte[], byte[]>(KafkaNativeSample.getClientConfiguration(broker, apiKey, true));
	}
	...
	public void run()
	{
		...
		while(!closing)
		{
			...
			try
			{
				...
				// After a predefined number of messages are received, shut down.
